...
import cvxpy as cp
from EDAspy.optimization import EDAPerm
import time
import argparse
...
parser.add_argument("model", type=str, default="gmm", help="Probability distribution model (mm or gmm)")
parser.add_argument("estimation", type=str, default="approx", help="Estimation (approx or exact)")
parser.add_argument("distance", type=str, default="hamming", help="Distance (kendall, cayley, ulam or hamming)")
...
# python .\EDAPerm.py 10 cars gmm approx hamming
...
alpha = 0.5
#model = 'gmm' #mm, gmm
#estimation = 'approx' # approx, exact
#distance = 'hamming' #kendall, cayley, ulam, hamming
edas_results = []
edaperm = EDAPerm(size_gen=size_gen, max_iter=max_iter, dead_iter=dead_iter, n_variables=n_objects, alpha=alpha, model=model, distance=distance, estimation=estimation)
start_time = time.time()
for index, structure in enumerate(structures_list[:n_samples]):
    # Crear cod y u_dist_matrix_variables
    ...
    def fitness_function(order: list):
        # Calcular matriz ultramétrica del individuo
    ...
    edas_results.append(edaperm.minimize(fitness_function, True))
...
for index, eda_result in enumerate(edas_results):
    if eda_result.best_cost < fitness_best_result:
        ...
        best_individual = eda_result.best_ind
        best_individual_index = index
    fitness_all_costs.append(eda_result.best_cost)

txt_file = url + "\\" + dataset_name + "N" + str(n_objects) + ".txt"
with open(txt_file, 'w') as f:
    f.write(','.join(map(str, fitness_all_costs)))

fig, ax = plt.subplots()
ax.plot(fitness_all_costs)
ax.set_xlabel('Coste')
ax.set_ylabel('EDA')
ax.set_title(dataset_name + ", n=" + str(n_objects))

plt.savefig(dataset_name + "N" + str(n_objects))
plt.close(fig)



with open(url + "\\ExecN" + str(n_objects) + ".txt", 'a') as f:
    f.write("n_objects: {}, n_samples: {}, size_gen: {}, max_iter: {}, dead_iter: {}, alpha: {}\n".format(n_objects, n_samples, size_gen, max_iter, dead_iter, alpha))
    f.write("{} {} {}\n".format(model, estimation, distance))
